//----------------------------------------------------------------------------
#ifndef TauHelperFunctions_15763
#define TauHelperFunctions_15763
//----------------------------------------------------------------------------
...
double GetMinRadius(FourVector P1, FourVector P2, FourVector P3)   // in eta-phi space
{
   ...
   for(int i1 = 0; i1 <= 1; i1++)
   {
      for(int i2 = 0; i2 <= 1; i2++)
      {
         ...
         {
            double AveragePhi = (Phi1 + Phi2 + Phi3 + (i1 + i2 + i3) * 2 * M_PI) / 3;

            double Residual2 = (AveragePhi - Phi1 - i1 * 2 * M_PI) * (AveragePhi - Phi1 - i1 * 2 * M_PI)
               + (AveragePhi - Phi2 - i2 * 2 * M_PI) * (AveragePhi - Phi2 - i2 * 2 * M_PI)
               + (AveragePhi - Phi3 - i3 * 2 * M_PI) * (AveragePhi - Phi3 - i3 * 2 * M_PI);
   ...
   return GetMinRadius(Eta1, Phi1 + Best1 * 2 * M_PI, Eta2, Phi2 + Best2 * 2 * M_PI, Eta3, Phi3 + Best3 * 2 * M_PI);
}
//----------------------------------------------------------------------------
